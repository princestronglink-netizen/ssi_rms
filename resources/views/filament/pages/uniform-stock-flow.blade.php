...
.sf-type-block-total {
    ...
    color:var(--cl-text,#0e1b34);
    background:rgba(22,109,245,0.08); color:rgb(22,109,245);
    padding:1px 8px; border-radius:6px;
}

/* ── Size rows — plain label + number, no bars ───────────────────── */
...
.sf-size-name {
    font-size:11.5px; color:var(--cl-text-3,#7f96b6);
    font-family:'DM Sans',sans-serif; min-width:40px;
}
.sf-size-qty {
...
.sf-gen-report-btn svg { flex-shrink:0; }
...
.sf-modal-overlay.sf-modal--open .sf-modal {
...
.sf-modal-section-title::after {
    content:''; flex:1; height:1px;
    background:var(--cl-border,rgba(22,109,245,0.1));
}

.sf-date-row { display:grid; grid-template-columns:1fr 1fr; gap:12px; }

.sf-report-checkboxes {
...
.sf-report-check.sf-check--active .sf-report-check-box {
    background:rgb(22,109,245); border-color:rgb(22,109,245);
}
...
.sf-report-section-chip.sf-chip--active {
...
.sf-modal-footer {
    ...
    border-top:1px solid var(--cl-border,rgba(22,109,245,0.1));
...
.sf-modal-cancel {
    ...
    border-radius:10px; border:1px solid var(--cl-border-strong,rgba(22,109,245,0.2));
                            ...
                            <td style="min-width:280px;">
                                @if(!empty($row['issuance_types']))
                                    <div class="sf-issuance-types-wrap">
                                        @php
                                            $typeMax = collect($row['issuance_types'])->max('subtotal') ?: 1;
                                        @endphp
            ...
            <div class="sf-modal-section">
                ...
                <div class="sf-report-checkboxes">
                    <label class="sf-report-check sf-check--active" data-section="stock_summary">
                        ...
                        <div class="sf-report-check-box">
                            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg>
                        </div>
                        <div>
                            <div class="sf-report-check-label">Current Stock</div>
                            <div class="sf-report-check-desc">Per item & variant on-hand quantities</div>
                        </div>
                    </label>
                    <label class="sf-report-check sf-check--active" data-section="issuances">
                        ...
                        <div>
                            <div class="sf-report-check-label">Issuances</div>
                            <div class="sf-report-check-desc">Issued uniforms by type, item, size</div>
                        </div>
                    </label>
                    <label class="sf-report-check sf-check--active" data-section="restocks">
                        ...
                        <div>
                            <div class="sf-report-check-label">Restocks</div>
                            <div class="sf-report-check-desc">Delivered restock items & quantities</div>
                        </div>
                    </label>
                    <label class="sf-report-check sf-check--active" data-section="returns">
                        ...
                        <div>
                            <div class="sf-report-check-label">Returns</div>
                            <div class="sf-report-check-desc">Returned items added back to stock</div>
                        </div>
                    </label>
                    <label class="sf-report-check sf-check--active" data-section="issuance_by_site">
                        ...
                        <div>
                            <div class="sf-report-check-label">By Site</div>
                            <div class="sf-report-check-desc">Issuance grouped per site location</div>
                        </div>
                    </label>
                    <label class="sf-report-check" data-section="issuance_by_person">
                        ...
                        <div>
                            <div class="sf-report-check-label">By Person</div>
                            <div class="sf-report-check-desc">Individual recipient issuance records</div>
                        </div>
                    </label>
                </div>
            </div>
...
(function () {
    ...
    function openModal() {
        // Pre-fill dates from dashboard filters
    ...
    document.querySelectorAll('.sf-report-preset').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (preset === 'this_year')    { f = y + '-01-01'; }
                ...
                f = new Date(y, qStart, 1).toISOString().split('T')[0];
            }
            else if (preset === 'last_quarter') {
                const qStart = Math.floor(m / 3) * 3 - 3;
                f = new Date(y, qStart, 1).toISOString().split('T')[0];
                t = new Date(y, qStart + 3, 0).toISOString().split('T')[0];
            }
            else if (preset === 'last_6months') {
                f = new Date(y, m - 6, 1).toISOString().split('T')[0];
            }
            ...
            document.querySelectorAll('.sf-report-preset').forEach(b => {
                b.style.background = '';
                b.style.color = 'var(--cl-text-3,#7f96b6)';
                b.style.borderColor = 'var(--cl-border,rgba(22,109,245,0.15))';
            });
            btn.style.background = 'rgba(22,109,245,0.1)';
            btn.style.color = 'rgb(22,109,245)';
            btn.style.borderColor = 'rgba(22,109,245,0.3)';
        });
    });
    ...
    generateBtn?.addEventListener('click', function () {
        const sections = Array.from(document.querySelectorAll('.sf-report-check[data-section].sf-check--active'))
        ...
        if (!sections.length) {
            alert('Please select at least one report section.'); return;
        }
