            ...
            ->recordActions([
                ActionGroup::make([
                    ...
                    Action::make('edit')
                        ...
                        ->fillForm(function (SmePurchaseOrder $record): array {
                            $record->loadMissing('purchaseOrderItems.smeItem', 'purchaseOrderItems.smeItemVariant');

                            return [
                                'site_id'    => $record->site_id,
                                'status'     => $record->status,
                                'po_number'  => $record->po_number,
                                'po_date'    => $record->po_date?->toDateString(),
                                'po_file_path' => $record->po_file_path,
                                'note'       => $record->note,
                                'purchaseOrderItems' => $record->purchaseOrderItems->map(fn ($item) => [
                                    'sme_item_id'         => $item->sme_item_id,
                        ...
                        ->action(function (SmePurchaseOrder $record, array $data, Action $action): void {
                            ...
                            if ($status === 'pending' && !empty($issues)) {
                                ...
                                if (!$acknowledged) {
                                    Notification::make()
                                        ...
                                        ->body('Some items exceed available stock. Tick the confirmation checkbox before saving.')
                                        ->warning()
                                        ->persistent()
                                        ->send();
                            ...
                            DB::transaction(function () use ($record, $data): void {
                                ...
                                $record->purchaseOrderItems()->delete();
                                ...
                                \App\Models\SmePurchaseOrderLog::create([
                                    'sme_purchase_order_id' => $record->id,
                                    'user_id'               => \Illuminate\Support\Facades\Auth::id(),
                                    'action'                => 'edited',
                                    'status_from'           => $record->getOriginal('status'),
                                    'status_to'             => $data['status'] ?? $record->status,
                                    'note'                  => null,
                                ]);
                            });
