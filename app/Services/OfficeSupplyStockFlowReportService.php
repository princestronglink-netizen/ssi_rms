...
class OfficeSupplyStockFlowReportService
{
    ...
    public static function generate(array $payload): string
    {
        ...
        $toFmt    = $to   ? $fmtDate($to)   : '—';
        $title    = 'Office Supply Stock Flow Report';
        ...
        if (in_array('requests', $sections) && !empty($payload['req_by_item'])) {
            $bodyHtml .= self::renderReqByItem($payload['req_by_item']);
        }
        if (in_array('req_by_requester', $sections) && !empty($payload['req_by_requester'])) {
            $bodyHtml .= self::renderReqByRequester($payload['req_by_requester']);
        }
        if (in_array('req_by_number', $sections) && !empty($payload['req_by_number'])) {
            $bodyHtml .= self::renderReqByNumber($payload['req_by_number']);
        }
    ...
    private static function renderStockSummary(array $data): string
    {
        ...
        foreach ($grouped as $itemName => $variants) {
            ...
            foreach ($variants as $v) {
                ...
                $rows .= "<td class='td-center'>{$variant}</td><td class='td-right'><strong>{$qty}</strong></td></tr>";
            }
            $alt = !$alt;
        }
    ...
    private static function renderReqByItem(array $data): string
    {
        $rows = '';
        $alt  = false;
        ...
        foreach ($data as $row) {
            ...
            $typeHtml = '';
            foreach ($row['breakdown'] ?? [] as $typeName => $typeData) {
                $sub      = number_format((int) ($typeData['subtotal'] ?? 0));
                $tName    = e(ucfirst($typeName));
                $variants = '';
                foreach ($typeData['variants'] ?? [] as $vName => $vQty) {
                    $variants .= '<span class="chip">' . e($vName ?: '—') . ': ' . number_format((int)$vQty) . '</span>';
                }
                $typeHtml .= "<div class='type-row'><span class='pill'>{$tName}</span> <strong>{$sub} units</strong> {$variants}</div>";
            }
        ...
        return <<<HTML
        <div class="section">
            <div class="section-title">Supply Requests by Item</div>
            <table>
                <thead><tr>
                    <th>Item</th>
                    <th>Breakdown by Variant</th>
    ...
    private static function renderReqByRequester(array $data): string
    {
        $rows = '';
        $alt  = false;
        ...
        foreach ($data as $row) {
            ...
            foreach ($row['breakdown'] ?? [] as $typeName => $typeData) {
                ...
                $typeHtml .= "<div class='type-row'><span class='pill'>{$tName}</span> <strong>{$sub} units</strong> {$items}</div>";
            }
        ...
        return <<<HTML
        <div class="section">
            <div class="section-title">Supply Requests by Requester</div>
            <table>
                <thead><tr>
                    <th>Requester</th>
    ...
    private static function renderReqByNumber(array $data): string
    {
        $rows = '';
        $alt  = false;
        ...
        foreach ($data as $row) {
            ...
            $rows .= "<tr style='{$bg}'>
                <td class='td-main'><strong>{$label}</strong><br><span class='meta'>{$requester}</span></td>
                <td>{$typeHtml}</td>
                <td class='td-right'><strong>{$total}</strong></td>
            </tr>";
            $alt = !$alt;
        ...
        return <<<HTML
        <div class="section">
            <div class="section-title">Supply Requests by Request Number</div>
            <table>
                <thead><tr>
                    <th>Request #</th>
                    <th>Items Breakdown</th>
    ...
    private static function wrapDocument(
        string $title, string $fromFmt, string $toFmt, string $genTime, string $bodyHtml
    ): string {
        $safeTitle = e($title);
...
.a4 { width:210mm; background:#fff; box-shadow:0 8px 32px rgba(0,0,0,.22); border-radius:2px; overflow:hidden; }
.report-page { padding:8mm 10mm 10mm; }

.co-header { display:flex; flex-direction:column; align-items:center; justify-content:center; padding:2mm 0; border-bottom:2.5px solid #1e3a5f; text-align:center; }
.co-logo-wrap { width:220px; height:90px; display:flex; align-items:center; justify-content:center; overflow:hidden; }
.co-logo-img { width:100%; height:100%; object-fit:contain; display:block; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
.co-tagline { font-size:8.5pt; color:#475569; letter-spacing:.01em; margin-top:-8px; line-height:1; }

.dept-row { width:100%; border-collapse:collapse; border:1.5px solid #000; border-top:none; }
...
.date-meta { width:100%; border-collapse:collapse; border:1px solid #ccc; border-top:none; margin-bottom:6mm; }
...
.kpi { flex:1; min-width:40mm; padding:3mm 4mm; border-radius:4px; border-left:4px solid transparent; }
...
.section-title { font-size:11pt; font-weight:700; color:#1a237e; padding:2mm 3mm; margin-bottom:0; background:#e8edf7; border-left:4px solid #1a237e; text-transform:uppercase; letter-spacing:.04em; }

table { width:100%; border-collapse:collapse; font-size:9pt; }
thead tr { background:#1a237e; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
thead th { padding:2mm 3mm; text-align:left; color:#fff; font-size:8pt; font-weight:700; text-transform:uppercase; letter-spacing:.05em; border:1px solid #1a237e; }
...
.pill { display:inline-block; padding:1px 6px; border-radius:4px; background:#e0eaff; color:#1a237e; font-size:7.5pt; font-weight:700; text-transform:uppercase; letter-spacing:.04em; margin-right:4px; }
.chip { display:inline-block; background:#f1f5f9; color:#475569; padding:1px 5px; border-radius:3px; font-size:7.5pt; margin:2px 2px 0 0; }

.addr-footer { text-align:center; font-size:7.5pt; color:#555; padding-top:3mm; border-top:1px solid #ccc; margin-top:8mm; line-height:1.6; }
...
@media print {
    ...
    .kpi-grid,.kpi,thead tr,.pill,.chip { -webkit-print-color-adjust:exact; print-color-adjust:exact; }
}
</style>
</head>
<body>
...
<div class="toolbar">
    <div class="tbar-l">
        <div>
            <div class="tbar-title">🗂️ {$safeTitle}</div>
...
<div class="report-page">
    ...
    <table class="dept-row">
        <tr>
            ...
            <td class="report-label">Office Supply Stock Flow Report</td>
        </tr>
    </table>
